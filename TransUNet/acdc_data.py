...
def prepare_data(
    input_folder, output_file, mode, size, target_resolution, split_test_train=True
):
    ...
    for tt in ["test", "train"]:
        ...
        hdf5_file.create_dataset(
            "weight_%s" % tt, data=np.asarray(weight_list[tt], dtype=np.float32)
        )
        hdf5_file.create_dataset(
            "height_%s" % tt, data=np.asarray(height_list[tt], dtype=np.float32)
        )
        hdf5_file.create_dataset(
            "patient_id_%s" % tt, data=np.asarray(patient_id_list[tt], dtype=np.uint8)
        )
        hdf5_file.create_dataset(
            "cardiac_phase_%s" % tt,
            data=np.asarray(cardiac_phase_list[tt], dtype=np.uint8),
        )
            ...
            if mode == "3D":
                ...
                if write_buffer >= MAX_WRITE_BUFFER:
                    ...
                    _write_range_to_hdf5(
                        data, train_test, img_list, mask_list, counter_from, counter_to
                    )
...
def _write_range_to_hdf5(
    hdf5_data, train_test, img_list, mask_list, counter_from, counter_to
):
    """
...
def load_and_maybe_process_data(
    ...
    if not os.path.exists(data_file_path) or force_overwrite:
        ...
        logging.warning(
            "WARNING: Your preprocessed data file is smaller than 10MB. It is likely that something went "
...
if __name__ == "__main__":
    ...
    d = load_and_maybe_process_data(
        input_folder, preprocessing_folder, "2D", (212, 212), (1.36719, 1.36719)
    )
